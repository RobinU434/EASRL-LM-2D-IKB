...
In this section we are going to present the results of training the VAE with the imitation loss to pre-calculated solutions from \algoref{alg:CCD}. 
...
All in all the distance loss functions in \figref{} yield almost the same examples as the distance loss functions in \figref{fig:VAE_latent/reconstruction_4}. Additional to the distance loss we have got also an imitation loss between the computed action 
...
The distance loss results over different amount of joints are very similar compared to \figref{} \todo{reference to distance plot from VAE setting.}. We can also see a decrease in performance while increase the number of joints. 
