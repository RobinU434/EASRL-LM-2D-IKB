...
class LearningProcess(ABC):
    ...
    def _build_save_dir(self, model_entity: str, **kwargs) -> str:
        if "checkpoint" in kwargs.keys():
            save_dir = "/".join(kwargs["checkpoint"].split("/")[:-1])
        elif "subdir" in kwargs.keys():
            save_dir = f"results/{model_entity}/{kwargs['subdir']}/{self._config['num_joints']}_{int(time.time())}"
        else:
