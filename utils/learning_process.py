...
class LearningProcess(ABC):
    def __init__(
        ...
        **kwargs
        ...
        self._save_dir = self._build_save_dir_path(self._model_entity_name, **kwargs)
    ...
    def _build_save_dir_path(self, model_entity: str, **kwargs) -> str:
        ...
        if "checkpoint" in kwargs.keys():
            save_dir = "/".join(kwargs["checkpoint"].split("/")[:-1])
        elif "subdir" in kwargs.keys():
            save_dir = f"results/{model_entity}/{kwargs['subdir']}/{self._config['n_joints']}_"
            try:
